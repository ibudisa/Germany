...
namespace MarketplaceWebServiceProducts.Samples
{
    ...
    public class GetCompetitivePricingForSKUSample
    {
        ...
        public static void InvokeGetCompetitivePricingForSKU(MarketplaceWebServiceProducts service, GetCompetitivePricingForSKURequest request)
        {
            try 
            {
                ...
                Console.WriteLine("        GetCompetitivePricingForSKUResponse");
                List<GetCompetitivePricingForSKUResult> getCompetitivePricingForSKUResultList = response.GetCompetitivePricingForSKUResult;
                foreach (GetCompetitivePricingForSKUResult getCompetitivePricingForSKUResult in getCompetitivePricingForSKUResultList)
                {
                    Console.WriteLine("            GetCompetitivePricingForSKUResult");
                if (getCompetitivePricingForSKUResult.IsSetSellerSKU()) {
                    Console.WriteLine("        SellerSKU");
                    Console.WriteLine();
                    Console.WriteLine("                {0}",  getCompetitivePricingForSKUResult.SellerSKU);
                    Console.WriteLine();
                } 
                if (getCompetitivePricingForSKUResult.IsSetstatus()) {
                    Console.WriteLine("        status");
                    Console.WriteLine();
                    Console.WriteLine("                {0}",  getCompetitivePricingForSKUResult.status);
                    Console.WriteLine();
                } 
                    if (getCompetitivePricingForSKUResult.IsSetProduct())
                    {
                        Console.WriteLine("                Product");
                        Product  product = getCompetitivePricingForSKUResult.Product;
                    ...
                    if (getCompetitivePricingForSKUResult.IsSetError())
                    {
                        Console.WriteLine("                Error");
                        Error  error = getCompetitivePricingForSKUResult.Error;
                        if (error.IsSetType())
                        {
